...
- **品質保証 (CI)**: コードを変更した後は必ず `make ci` を実行し、すべての静的解析とテストが成功することを確認すること。エラーが発生した場合はただちに修正すること。
...
| Command        | Description                                                           |
| -------------- | --------------------------------------------------------------------- |
| `make setup`   | venv の作成 + pip 依存インストール + `.env` テンプレート作成          |
...
- **リンター** — 未設定。CI は `make test` のみ実行するため、フォーマット等の自動修正は不要。
- SQLite DB は `data/reviews.db` に自動作成される。
- テスト時は `conftest.py` で `REFIX_TURSO_DATABASE_URL` / `REFIX_TURSO_AUTH_TOKEN` を自動 unset するため、常にローカル SQLite を使用する。
