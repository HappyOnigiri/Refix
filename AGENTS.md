...
- **ワークフローテンプレートのバージョン管理**: `scripts/templates/run-refix.yml` および `scripts/templates/run-refix.ja.yml` を変更した場合、両ファイルの `Template version` を同じ値に更新すること。バージョン形式は `MAJOR.MINOR.PATCH`。
  - PATCH を +1（例: 1.3.0 → 1.3.1）: コメント・ドキュメントのみの変更
  - MINOR を +1・PATCH をリセット（例: 1.3.1 → 1.4.0）: 機能追加・動作変更（後方互換性あり）
  - MAJOR を +1・MINOR/PATCH をリセット（例: 1.4.0 → 2.0.0）: 後方互換性を失う変更（既存パラメータの削除・リネームなど）
  - **タイミング**: バージョンは PR 単位で1度だけ上げる。同一 PR 内で複数回テンプレートを変更しても、バージョンは最初の変更時に1度だけインクリメントすればよい。
  - **例外**: ja のみドキュメント変更した場合など、一方のみ更新してバージョンがずれることがある（例: ja: 1.3.2 / en: 1.3.1）。その場合、次回両方を更新する際は大きい方を基準にインクリメントして揃える（例: 次の PATCH 更新では ja: 1.3.3 / en: 1.3.3）。
