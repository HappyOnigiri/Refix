...
`--repo` と `--pr` を指定することで、設定ファイルに `repositories` を記載しなくても特定の Pull Request を直接処理できます。手動テストや単発の修正に便利です。
...
- `.refix.yaml` の `repositories` フィールドは不要です。
- `--config` で指定したファイルが存在すれば、トップレベルの設定（`models`、`ci_log_max_lines` など）はそこから読み込まれますが、`repositories` は無視されます。
- 設定ファイルが存在しない場合は、組み込みのデフォルト値が使われます。
- `--repo` と `--pr` は必ずセットで指定してください。
